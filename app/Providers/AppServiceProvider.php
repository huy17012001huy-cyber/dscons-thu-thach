    ...
    public function boot(): void
    {
        ...
        if (! $this->app->runningInConsole()) {
            try {
                ...
                if ($v = $cfg->get('mail_driver')) {
                    config(['mail.default' => $v]);
                }
                if ($v = $cfg->get('resend_api_key')) {
                    config(['services.resend.key' => $v]);
                }
                if ($v = $cfg->get('brevo_api_key')) {
                    config(['services.brevo.key' => $v]);
                }
                if ($v = $cfg->get('mail_from_address')) {
                    config(['mail.from.address' => $v]);
                }
                if ($v = $cfg->get('mail_from_name')) {
                    config(['mail.from.name' => $v]);
                }

                if ($v = $cfg->get('sepay_bank_account')) {
                    config(['services.sepay.bank_account' => $v]);
                }
                if ($v = $cfg->get('sepay_bank_name')) {
                    config(['services.sepay.bank_name' => $v]);
                }
                if ($v = $cfg->get('sepay_webhook_token')) {
                    config(['services.sepay.webhook_token' => $v]);
                }
            } catch (\Throwable $e) {
