...
class AdminChallenges extends Component
{
    ...
    public ?TemporaryUploadedFile $expCover = null;
    ...
    public ?TemporaryUploadedFile $taskRewardFile = null;
    ...
    public function saveExpedition(): void
    {
        ...
        if ($this->expCover) {
            $path = $this->expCover->store('challenge/covers', 'public');
            if (is_string($path)) {
     ...
     */
    /**
     * @return array{total_users: int, total_questions: int, per_question: array<int, array{total: int, correct: int, wrong: int, rate: float|int}>, users: array<int, array{id: int, name: string, username: string|null, answered: int, score: int, completion: float|int}>}
     */
    public function getQuizReportData(?ChallengeTask $task): array
    {
        ...
        $users = $userRows->map(fn ($u) => [
            ...
            'score' => (int) $u->score,
            'completion' => (int) $u->answered / $totalQuestions,
        ])->values()->all();
