...
class ChallengeRewardController extends Controller
{
    ...
    public function download(string $slug, int $day)
    {
        $user = Auth::user();
        if (!$user) abort(403);
        ...
        if (!$task->reward_file_path) abort(404);
        ...
        if (!$user->is_admin) {
            ...
            $hasApprovedSubmission = DB::table('challenge_task_completions')
                ...
                ->exists();
            if (!$hasApprovedSubmission) abort(403);
        }

        if (!Storage::disk('local')->exists($task->reward_file_path)) abort(404);
