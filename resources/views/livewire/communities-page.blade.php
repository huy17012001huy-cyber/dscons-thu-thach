...
<style>
    .discovery-page { max-width: 1120px; margin: 0 auto; padding: 34px clamp(16px, 3vw, 30px) 64px; color: var(--text); }
    ...
    .discovery-heading h1 { margin: 0; color: var(--text); font-size: clamp(26px, 3vw, 32px); font-weight: 800; letter-spacing: -.045em; line-height: 1.12; }
    .discovery-heading p { max-width: 620px; margin: 9px 0 0; color: #627D90; font-size: 13px; line-height: 1.6; }
    ...
    .discovery-search input:focus-visible { border-color: var(--green); box-shadow: 0 0 0 3px rgba(31,119,190,.13); }
    ...
    .discovery-section h2 { margin: 0; color: var(--text); font-size: 17px; letter-spacing: -.02em; }
    ...
    .discovery-card:focus-within { border-color: #1F77BE; box-shadow: 0 0 0 3px rgba(31,119,190,.15), 0 10px 24px rgba(18,59,89,.09); }
    .discovery-card-link { position: absolute; z-index: 1; inset: 0; border-radius: inherit; }
    .discovery-card-link:focus-visible { outline: 3px solid rgba(31,119,190,.45); outline-offset: -3px; }
    .discovery-card-cover { position: relative; aspect-ratio: 16 / 7; overflow: hidden; background: #E1F4F7; }
    .discovery-card-cover::after { content: ''; position: absolute; inset: 0; background: rgba(255,255,255,.14); }
    ...
    .discovery-card-info { position: relative; z-index: 2; display: flex; flex: 1; flex-direction: column; padding: 16px; pointer-events: none; }
    ...
    .discovery-create-card { display: grid; min-height: 100%; box-sizing: border-box; place-items: center; padding: 24px; border: 1px dashed #AACCD7; border-radius: 18px; background: #F7FBFC; color: var(--green); text-align: center; text-decoration: none; transition: background .17s ease, border-color .17s ease; }
    .discovery-create-card > div { display: grid; width: min(100%, 220px); justify-items: center; align-content: center; }
    .discovery-create-card:hover { border-color: var(--green); background: #EDF8FA; }
    .discovery-create-card:focus-visible { outline: none; border-color: var(--green); box-shadow: 0 0 0 3px rgba(31,119,190,.18); }
    .discovery-create-card svg { width: 30px; height: 30px; margin: 0 auto 11px; }
    .discovery-create-card strong { display: block; font-size: 14px; line-height: 1.3; }
    .discovery-create-card span { display: block; margin-top: 5px; color: var(--text-muted); font-size: 11px; line-height: 1.5; }
    .discovery-empty { display: grid; justify-items: center; gap: 8px; padding: 48px 24px; border: 1px dashed #BFD5DE; border-radius: 14px; background: #fff; color: var(--text-muted); text-align: center; }
    .discovery-empty strong { color: var(--text); font-size: 15px; }
    @media (max-width: 1050px) { .discovery-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
    @media (max-width: 700px) { .discovery-page { padding: 20px 14px 42px; } .discovery-heading { display: block; padding-bottom: 20px; } .discovery-create { display: flex; justify-content: center; margin-top: 16px; } .discovery-search-row { display: block; margin: 16px 0 22px; } .discovery-count { display: block; margin: 9px 0 0; } .discovery-section { display: block; margin-bottom: 12px; } .discovery-section span { display: block; margin-top: 5px; } .discovery-grid { grid-template-columns: 1fr; gap: 13px; } }
</style>
    ...
    <div class="discovery-search-row">
        ...
        <span class="discovery-count">{{ $communities->count() }} cộng đồng đang hoạt động</span>
    </div>

    <div class="discovery-section">
        <h2>Danh sách cộng đồng</h2>
        <span>Bấm vào cộng đồng để xem giới thiệu</span>
    </div>

    @if($communities->isEmpty())
        <div class="discovery-empty">Chưa có cộng đồng phù hợp. Hãy thử một từ khóa khác.</div>
    @else
        <div class="discovery-grid">
            @foreach($communities as $community)
                ...
                <article class="discovery-card">
                    <a href="{{ $communityUrl }}" class="discovery-card-link" aria-label="{{ $isJoined ? 'Vào' : 'Xem' }} cộng đồng {{ $community->name }}"></a>
            ...
            @if($communities->count() < 3)
                <a href="{{ route('community.create') }}" class="discovery-create-card">
                    <div>
                        ...
                        <strong>Tạo cộng đồng mới</strong>
                        <span>Gửi đề xuất để nền tảng duyệt và mở cộng đồng của bạn.</span>
                    </div>
                </a>
            @endif
        </div>
