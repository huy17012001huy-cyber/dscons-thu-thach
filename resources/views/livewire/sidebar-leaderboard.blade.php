<div class="card" style="padding:0.875rem;">
    <div class="flex justify-between items-center mb-2">
        ...
        <a href="{{ route('leaderboard') }}" style="font-size:0.7rem; color:#d17856; font-weight:600;">Xem tất cả →</a>
    </div>
    @foreach($top as $i => $user)
    <div class="flex items-center gap-2 py-1.5" style="{{ !$loop->last ? 'border-bottom:1px solid #E1E1E1;' : '' }}">
        <span style="font-size:0.75rem; font-weight:700; width:18px; text-align:center; color:{{ $i === 0 ? '#d17856' : ($i === 1 ? '#94A3B8' : ($i === 2 ? '#B45309' : '#5C5C66')) }};">{{ $i === 0 ? '#1' : ($i+1) }}</span>
        ...
        <div style="flex:1; min-width:0;">
            ...
            <p style="font-size:0.65rem; color:#5C5C66;">{{ number_format($user->xp) }} EXP</p>
        </div>
